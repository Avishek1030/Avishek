...
client = openai.OpenAI(api_key=oc.getenv("Avishek_key"))
...
def generate_resume_with_reportlab(data):
   ...
   # Add Hobbies
    if data.get("Hobbies"):
        hobbies = [
            f"{'Hobbies'} - <font color='darkblue'></font> )"
            for Hobbies in data["Hobbies"]
        ]
        add_section("Hobbies", Hobbies, content, normal_style, title_color=colors.darkblue)
    # Add Work Experience
...
# Section: Hobbies
st.header("🎮 Hobbies")
hobbies = st.text_area("Hobbies", placeholder="Enter your hobbies separated by commas")
